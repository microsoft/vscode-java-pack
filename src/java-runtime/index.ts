...
export async function findJavaRuntimeEntries(): Promise<{
  ...
  try {
    ...
    if (!javaVersion || javaVersion < 11) {
      javaHomeError = `Java 11 or more recent is required by the Java language support (redhat.java). Preferred JDK "${javaDotHome}" (version ${javaVersion}) doesn't meet the requirement. Please specify or install a recent JDK.`;
    }
  } catch (error) {
