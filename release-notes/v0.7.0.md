...
To debug Maven goals, just right click on a goal and go debugging. The Maven extension will call the Java debugger with the right parameters. This is handy and super time-saving.
...
If you experienced the problem of setting up the JDK, we've got you covered. The JDK Acquisition Guide follows the same logic used by the Java language server to check for a JDK installation from your machine. When the check fails, the guide will automatically show and recommend a compatible JDK distribution according to your OS and CPU architecture.
