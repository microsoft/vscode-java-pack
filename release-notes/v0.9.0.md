...
Tired of waiting on the language server to start? Now the wait is over. With the help the new Syntax Server, you will be able to use a fair amount of features within 10 seconds:

![Syntax Server Startup](v0.9.0/syntax-server-startup.gif)

This is useful especially when you open a project for the first time. Instead of waiting for all the dependencies to be downloaded, you can use the features listed below:
...
Syntax Mode is more than just syntax info. It actually allows you to browse the known types that are defined in your *local source*, or in the *JDK*. You can navigate to the definition or the references to those types, and hover to view the documentations.
...
Syntax Mode is also the foundation of the Syntax Server. It is tolerent and is capable of working with missing types. So in Syntax Mode, you don't need to wait for the depencencies to be resolved.
...
![Call Hierarchy](v0.9.0/call-hierarchy.gif)
...
Tons of new code actions has been added to make your life easier. To access them, click on the `light bulb` 💡 whenever you see it. Or right click the editor view and pick "`Source Action...`". Here is a list of the newly added code actions:
...
![Reference JAR Files](v0.9.0/reference-jar-files.gif)
...
You can tell that glob pattern is supported. And here's more - you can  include/exclude certain files, and attach source JARs:
...
Our friends from Eclipse MicroProfile community published several extensions to enhance the development experience.
...
![Check Build Status](v0.9.0/check-build-status.gif)
...
![Go to Super Implementation](v0.9.0/go-to-super-implementation.gif)
